...
from .models import IdealFor,Occasion,Sleeve,Color,Brand,NeckType, Cart, Payment, Order,OrderItem
...
class CartConfiguration(admin.ModelAdmin):
    ...
    def get_tshirt(self , obj):
        ...
        tshirt_id  = tshirt.id 
        name = tshirt.name
        return format_html(f'<a href="/admin/shop/tshirt/{tshirt_id}/change/" target="_blank">{name}</a>')
...
class OrderConfiguration(admin.ModelAdmin):
    ...
    def payment(self , obj):
        ...
        return format_html(f'<a href="/admin/shop/payment/{payment_id}/change/" target="_blank">Click For Payment Information</a>')
